...
const AgCDPlaybookPublicPreview: React.FC = () => {
  const navigate = useNavigate();
  const [activePageTab, setActivePageTab] = useState<'home' | 'playbook'>('playbook');
  const [activeFilter, setActiveFilter] = useState<'orchestrator'>('orchestrator');
  ...
  const loadPrompts = () => {
    // Only show playbooks created in "Agentic routing public preview" flow
    const allPrompts = getAllPrompts();
    const filteredPrompts = allPrompts.filter(p => p.isPublicPreview === true);
    setPrompts(filteredPrompts);
  };

  const filteredPolicies = prompts.filter(policy => {
    // Filter by type - only orchestrator in public preview
    const matchesFilter = policy.type === 'Orchestrator';
  ...
  const handlePolicyClick = (policyId: string) => {
    navigate(`/agcd-preview/policy/${policyId}`);
  };
  ...
  const renderQueues = (policy: PromptData) => {
    ...
    } else if (policy.selectedProfiles && policy.selectedProfiles.length > 0) {
      if (expandedRows.has(policy.id)) {
        return (
          <div className="profiles-expanded-list">
            ...
            {policy.selectedProfiles.map((queue, index) => (
              <span
                key={index}
                className="profile-tag-item"
              >
                {queue.profileName}
              </span>
            ))}
    ...
    return <span className="no-profile-text">No queues selected</span>;
  };
    ...
    <main className="main-content agcd-playbook-page">
      <div className="playbook-page-wrapper">
        ...
        <div className="playbook-breadcrumb">
          <Link to="/agcd-preview" className="breadcrumb-link-style">Orchestration Agent</Link>
        ...
        <div className="search-and-new-container">
          ...
          <div className="action-buttons-container">
            <button className="new-playbook-button" onClick={() => navigate('/agcd-preview')}>
...
export default AgCDPlaybookPublicPreview;
